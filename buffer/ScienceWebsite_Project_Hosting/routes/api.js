...
router.post('/newResourceType/', function (request, response) {
    ...
    if (!requiredReady) {
        ...
        console.log(alert);
        responseWithCode(ReturnCode.clientError + methodCode + "001", alert, response)
    }
    else {
        flow.exec(
            function () {
                ...
                if (err) {
                    responseWithCode(ReturnCode.serviceError + methodCode + code, err, response);
                }
                else {
                    responseWithCode(ReturnCode.success, "New Resource Type was saved successfully as _id: " + saveResult._id, response);
                }
            }
        );
    }
        ...
        flow.exec(
            function () {
                ...
                if (err) {
                    ...
                    responseWithCode(ReturnCode.success, "New Department was saved successfully as _id: " + saveResult._id, response);
                }
            }
        );
    }
        ...
        flow.exec(
            function () {
                ...
                if (err) {
                    ...
                    responseWithCode(ReturnCode.success, "New Tag was saved successfully as _id: " + saveResult._id, response);
                }
            }
        );
    }
...
router.post('/newNews/', function (request, response) {
    ...
    if (!requiredReady) {
        ...
        console.log(alert);
        responseWithCode(ReturnCode.clientError + methodCode + "002", alert, response)
    }
    else if (!objectIdReady) {
        var alert = "Input Not Valid, check if some data is not ObjectID for MongoDB."
        console.log(alert);
        responseWithCode(ReturnCode.clientError + methodCode + "003", alert, response)
    }
    else {
        flow.exec(
            function () {
                ...
                if (err) {
                    ...
                    responseWithCode(ReturnCode.success, "New News was saved successfully as _id: " + saveResult._id, response);
                }
            }
        );
    }
        ...
        flow.exec(
            function () {
                ...
                if (err) {
                    ...
                    responseWithCode(ReturnCode.success, "News with _id: " + request.body.newsID + " has updated successfully.", response);
                }
            }
        );
    }
...
router.post('/getAllNewsPreview/', function (request, response) {
    var news = new News();
    var methodCode = "06";
        ...
        flow.exec(
            function () {
                ...
                if (err) {
                    ...
                    News_Control.getAllNews(new ObjectId(request.body.resourceId), new ObjectId(request.body.departmentId), false, this);
                }
            }, function (code, err, result) {
                if (err) {
                    responseWithCode(ReturnCode.serviceError + methodCode + code, err, response);
                    ...
                    responseWithCode("999999", result, response)
                }
            }
        );
    }
        ...
        flow.exec(
            function () {
                ...
                if (err) {
                    ...
                    responseWithCode("999999", obj, response)
                }
            }
        );
    }
