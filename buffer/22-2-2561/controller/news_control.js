...
module.exports = {
    ...
    getAllNews: function (resourceId, departmentId, isPreview, limitNum, isPosted, callback) {
        var today = new Date();

        var myquery = { $and: [{ "resourceId": resourceId }, { "departmentId": departmentId }] };
        var projection = { "resourceId": false, "departmentId": false }

        if (resourceId == "0" && departmentId == "0")
            myquery = {}
        ...
        if (isPreview == "true") {
            projection = { "_id": true, "topicShort": true, "detailShort": true, "topicPicture": true, "readCount": true, "isPinned": true };
        }
        if (isPosted == "true") {
